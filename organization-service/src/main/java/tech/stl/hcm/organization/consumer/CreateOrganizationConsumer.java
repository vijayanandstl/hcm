...
@TopicListener(
        topic = "${organization.kafka.topic}",
        groupId = "${organization.kafka.group-id}",
        valueType = OrganizationDTO.class,
        enableProperty = "organization.kafka.enable"
)
public class CreateOrganizationConsumer implements MessageHandler<OrganizationDTO> {
    ...
    public void handle(OrganizationDTO organizationDTO, String key) {
        try {
            ...
            log.info("Consumed organization: {}", organizationDTO.getName());
