...
@TopicListener(
        topic = "${candidateskill.kafka.topic}",
        groupId = "${candidateskill.kafka.group-id}",
        valueType = CandidateSkillDTO.class,
        enableProperty = "candidateskill.kafka.enable"
)
public class CreateCandidateSkillConsumer implements MessageHandler<CandidateSkillDTO> {
    ...
    public void handle(CandidateSkillDTO candidateSkill, String key) {
        try {
            ...
            log.info("Consumed candidate skill for candidate: {}", candidateSkill.getCandidateId());
            candidateSkillService.createCandidateSkill(candidateSkill);
        } finally {
            MDC.clear();
        }
    }
